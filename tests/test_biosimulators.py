...
class BioSimulatorsCombineTestCase(unittest.TestCase):
    ...
    def test_SmoldynOutputFile(self):
        output_file = smoldyn.biosimulators.data_model.SmoldynOutputFile(
            "out.txt", "/tmp/out.txt"
        )
    ...
    def test_init_smoldyn_simulation_from_configuration_file_handle_not_a_smoldyn_file(
        self,
    ):
    ...
    def test_read_write_smoldyn_simulation_configuration(self):
        ...
        config2 = smoldyn.biosimulators.combine.read_smoldyn_simulation_configuration(
            filename2
        )
    ...
    def test_get_smoldyn_instance_attr_or_run_algorithm_parameter_arg(self):
        ...
        param = smoldyn.biosimulators.combine.get_smoldyn_instance_attr_or_run_algorithm_parameter_arg(
            change
        )
        self.assertEqual(
            param,
        ...
        change = AlgorithmParameterChange(
            kisao_id="KISAO_0000488", new_value="not a float"
        )
        with self.assertRaises(ValueError):
            smoldyn.biosimulators.combine.get_smoldyn_instance_attr_or_run_algorithm_parameter_arg(
                change
            )

        change = AlgorithmParameterChange(
            kisao_id="KISAO_0000001", new_value="not a float"
        )
        with self.assertRaises(NotImplementedError):
            smoldyn.biosimulators.combine.get_smoldyn_instance_attr_or_run_algorithm_parameter_arg(
                change
    ...
    def test_add_smoldyn_output_file(self):
        ...
        output_file = smoldyn.biosimulators.combine.add_smoldyn_output_file(
            self.dirname, sim
        )
        ...
        smoldyn.biosimulators.combine.add_commands_to_smoldyn_output_file(
            sim,
            output_file,
            [
                smoldyn.biosimulators.data_model.SmoldynCommand(
                    command="molcount", type="E"
                ),
            ],
        )
    ...
    def test_add_smoldyn_output_files_for_sed_variables(self):
        ...
        smoldyn_output_files = (
            smoldyn.biosimulators.combine.add_smoldyn_output_files_for_sed_variables(
    ...
    def test_get_variable_results(self):
        ...
        self.assertEqual(
            set(results.keys()), set(["time", "red", "green", "greenBox", "greenHist"])
        )
    ...
    def test_get_variable_results_2d(self):
        filename = os.path.join(
            self.EXAMPLES_DIRNAME, "S4_molecules", "isotropic", "diffi.txt"
        )
        config = smoldyn.biosimulators.combine.read_smoldyn_simulation_configuration(
            filename
        )
        ...
        task = Task(
            ...
            simulation=UniformTimeCourseSimulation(
                ...
                algorithm=Algorithm(
                    ...
                    changes=[
                        AlgorithmParameterChange(
                            kisao_id="KISAO_0000488", new_value="10"
                        ),
                    ],
                ),
            ),
    ...
    def test_exec_sed_task_negative_initial_time(self):
        ...
        numpy.testing.assert_allclose(
            results["time"], numpy.linspace(-0.01, 0.09, 11), rtol=5e-6
        )
    ...
    def test_exec_sed_task_with_changes(self):
        task = Task(
            ...
            model=Model(
                id="model",
                source=os.path.join(
                    os.path.dirname(__file__), "fixtures", "lotvolt.txt"
                ),
                language=ModelLanguage.Smoldyn.value,
            ),
            simulation=UniformTimeCourseSimulation(
        ...
        for variable_id in variable_ids:
            variables.append(
                Variable(id=variable_id, target="molcount " + variable_id, task=task)
            )
        ...
        for variable_id in variable_ids:
            numpy.testing.assert_allclose(results2[variable_id], results[variable_id])
        ...
        for variable_id in variable_ids:
            model.changes.append(
                ModelAttributeChange(
                    target="fixmolcount " + variable_id,
        ...
        for variable_id in variable_ids:
            model.changes.append(
                ModelAttributeChange(target="killmol " + variable_id, new_value=None)
            )
        preprocessed_task = smoldyn.biosimulators.combine.preprocess_sed_task(
            task, variables
        )
        ...
        for variable_id in variable_ids:
            model.changes.append(
                ModelAttributeChange(target="killmol " + variable_id, new_value=0)
            )
        results3, _ = smoldyn.biosimulators.combine.exec_sed_task(
            task, variables, preprocessed_task=preprocessed_task
        )
        ...
        with self.assertRaisesRegex(
            NotImplementedError, "can only be changed during simulation preprocessing"
        ):
    ...
    def test_command_line_interface(self):
        ...
        with mock.patch.dict(os.environ, self._get_combine_archive_exec_env()):
            with smoldyn.biosimulators.__main__.App(
                argv=["-i", archive_filename, "-o", out_dir]
            ) as app:
                app.run()
        ...
        doc = SedDocument(
            ...
            data_generators=[
                DataGenerator(
                    id="data_gen_time",
                    variables=[
                        Variable(id="var_time", symbol=Symbol.time.value, task=task)
                    ],
                    math="var_time",
                ),
                DataGenerator(
                    id="data_gen_red",
                    variables=[
                        Variable(id="var_red", target="molcount red", task=task)
                    ],
                    math="var_red",
                ),
                DataGenerator(
                    id="data_gen_green",
                    variables=[
                        Variable(id="var_green", target="molcount green", task=task)
                    ],
                    math="var_green",
                ),
            ],
    ...
    def _get_combine_archive_exec_env(self):
        return {"REPORT_FORMATS": "h5"}

    def _assert_combine_archive_outputs(self, doc, out_dir):
        results = ReportReader().run(
            doc.outputs[0],
            out_dir,
            os.path.join("sim_1.sedml", doc.outputs[0].id),
        ...
        numpy.testing.assert_allclose(
            results["data_set_time"], numpy.linspace(0.1, 0.2, 11)
        )
        for result in results.values():
            self.assertEqual(result.shape, (11,))
            self.assertFalse(numpy.any(numpy.isnan(result)))
